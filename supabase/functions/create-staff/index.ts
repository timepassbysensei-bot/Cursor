...
// Same flow as create-student but for teacher accounts.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!["teacher", "admin"].includes(role)) return json({ error: "Invalid role" }, 400, cors);
    ...
    return json({
      ...
      message: `${role === "teacher" ? "Teacher" : "Admin"} account created. Share the temporary password securely.`,
    }, 200, cors);
  } catch (e) {
